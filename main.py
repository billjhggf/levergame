...
if partner:
  ...
  for i_episode in range(num_episodes):
      ...
      for t in count():
          ...
          step_done,action=select_action(state,step_done)
          ...
          reward_test_value=env.test(action)
      ...
      reward=evaluate(state2,n_iterations,N_history,partner_cycle,policy_net,policy_net,partner)
    ...
    if partner == False:
        ...
        for i_episode in range(num_episodes):
            ...
            for t in count():
                ...
                optimize_model(memory0, policy_net0, optimizer0, target_net0, n_observations, 0, 0.01)
                optimize_model(memory1, policy_net1, optimizer1, target_net1, n_observations, 0, 0.01)
                ...
                if done:
                    break
        path = 'L2regu/agent{}.h5'.format(k)

        torch.save(policy_net0, path)
all_policy_levers_arr = np.array(all_policy_levers)
savetxt('L2regu/policysequence.csv', all_policy_levers, delimiter=',')
